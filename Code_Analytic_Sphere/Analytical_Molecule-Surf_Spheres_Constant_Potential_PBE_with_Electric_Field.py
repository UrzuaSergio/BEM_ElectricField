...
def molecule_constant_potential(q, xq, phi02, Efield, r1, r2, R, kappa, E_1, E_2):
    ...
    print(special.kv(index2, kappa * 100000))
    ...
    C = numpy.zeros(N)
    for j in range(N):
        C[j] = (a[j]*k1[j] + i1[j]*(2*j +1)*numpy.sum(b * B[:,j]) - (q/(4*pi*E_1))*((rho**j)/(r1**(j+1))))/(r1**j)
    ...
    #a_inf = numpy.zeros(N)
    #for k in range(N):
    #    a_inf[k] = - (E_hat*(q/(4*pi*E_1))*((rho**k)/(r1**(k+2)))*(2*k+1))/(kappa*k1p[k])
    #    if k==1:
    ...
    print('E_solv_mol-surf: ', phi_h*C0*C1)
    #print('E_solv_mol_inf: ', phi_inf*C0*C1)
